...
public class PlayerBehavior : MonoBehaviour
{
	...
	/*
	if you want to add a control, is has to be added to teh following places:
	- PlayerBehavior
	- References
	- PlayerSettingsData
	...
	*/
	...
	readonly float runningAirAcceleration = 1.215f;			//how fast we accelerate to our "top speed" in the air running
	...
	readonly float skatingAirAcceleration = 0.35f;          //how fast we accelerate to our "top speed" in the air skating
	readonly float skatingGroundAcceleration = 1.215f;		//how fast we accelerate to our "top speed" on land skating (HAS TO BE SAME AS RUNNING AIR ACCELERATION OR MORE)
	...
	//wall running
	readonly float wallRunFriction = 5;
	readonly float wallRunTopSpeed = 16;
	readonly float wallRunAirAcceleration = Mathf.Infinity;
	readonly float wallRunGroundAcceleration = 6;
	...
	readonly float diveRegenRate = 4;
	...
	//thrust
	readonly float thrustForce = 2.25f;
	readonly float thrustMass = 50;
	readonly float thrustMaxSpeed = 24;
	readonly float thrustTurnAssist = 30;					//make it so the player can turn with thrust
	...
	void DoubleJump()
	{
		...
		if (Input.GetKeyDown(jumpButton) && doubleJumpReady && !isWallRunning)
		{
			...
			thisDoubleJumpSpeed = savedSpeed > doubleJumpMinLateralSpeed ? savedSpeed : doubleJumpMinLateralSpeed;
			if (thisDoubleJumpSpeed < MyLateralVelocity().magnitude)
				thisDoubleJumpSpeed = MyLateralVelocity().magnitude;
	...
	void Dive()
	{
		...
		//how close are we to our max dive?
		...
		if (Input.GetKeyDown(yeetButton) && diveReady && isSkating && !isGrounded && !isWallRunning)
		{
			...
			//get how close it is to the up vector
			...
			//give diveVector it's magnitude
			diveVector *= MyLateralVelocity().magnitude > currentDiveSpeed ? MyLateralVelocity().magnitude : currentDiveSpeed;

			//apply discharged dive penalty
			diveVector *= dischargedDivePenalty + ((1 - dischargedDivePenalty) * diveChargePercentage);

			//play our particle system and make sound (and stop superjump's)
